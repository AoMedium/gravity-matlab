...
%         gravityObjects = [gravityObjects, newObj];
    ...
    elseif cmdInput == "create image"
        image(CreateImage(50,50,gravityObjects,50,scale))
        disp("[!] Image created");
    else
...
function img = CreateImage(width, height, gravityObjects, iterations, scale)
    ...
    for i = 1:length(gravityObjects)
        ...
        if abs(x) < scale && abs(y) < scale
            pixelX = round(x/scale + width/2);
            pixelY = round(y/scale + width/2);
            
            img(pixelY,pixelX,:) = obj.colour;
        end
    end
